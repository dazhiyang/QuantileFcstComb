...
for(i in 1:length(stns))
{
  ...
  data <- tibble(read.table(file.path(dir0, paste0(stns[i],"_CQRA_2019_2020.txt")), sep = "\t", header = TRUE)) %>%
...
#################################################################################
weights <- tibble(read.table(file.path(dir0, paste0(stns[4],"_CQRA_Wnq.txt")), sep = "\t", header = TRUE)) 
