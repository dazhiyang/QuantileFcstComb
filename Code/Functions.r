#################################################################################
# These functions are largely based on the code by Daniel Wolffram
...
plot_coverage <- function(results) {
  ...
  if (band_type == "confidence") {
    bands_layer <- list(
      geom_ribbon(data = results, aes(x = quantile, ymin = l_5, ymax = l_95),
                  fill = "darkblue", alpha = 0.2),
      geom_ribbon(data = results, aes(x = quantile, ymin = u_5, ymax = u_95),
                  fill = "darkred", alpha = 0.2)
    )
  } else if (band_type == "confidence2") {
    bands_layer <- list(
      geom_ribbon(data = results, aes(x = quantile, ymin = l_25, ymax = l_75),
    ...
    bands_layer <- list(
      geom_ribbon(data = results, aes(x = quantile, ymin = lower50, ymax = upper50),
                  fill = "skyblue3", alpha = 0.3),
      geom_ribbon(data = results, aes(x = quantile, ymin = lower90, ymax = upper90),
